...
public abstract class BaseLayer implements DrawingContent, BaseKeyframeAnimation.AnimationListener, KeyPathElement {
    ...
    static BaseLayer forModel(Layer layer, LottieDrawable lottieDrawable2, LottieComposition lottieComposition) {
        switch (C05132.$SwitchMap$com$airbnb$lottie$model$layer$Layer$LayerType[layer.getLayerType().ordinal()]) {
            case 1:
    ...
    public void draw(Canvas canvas, Matrix matrix2, int i) {
        C0478L.beginSection(this.drawTraceName);
        if (!this.visible || this.layerModel.isHidden()) {
            C0478L.endSection(this.drawTraceName);
            return;
        }
        buildParentLayerListIfNeeded();
        C0478L.beginSection("Layer#parentMatrix");
        this.matrix.reset();
        this.matrix.set(matrix2);
        for (int size = this.parentLayers.size() - 1; size >= 0; size--) {
            this.matrix.preConcat(this.parentLayers.get(size).transform.getMatrix());
        }
        C0478L.endSection("Layer#parentMatrix");
        ...
        if (hasMatteOnThisLayer() || hasMasksOnThisLayer()) {
            C0478L.beginSection("Layer#computeBounds");
            ...
            C0478L.endSection("Layer#computeBounds");
            if (!this.rect.isEmpty()) {
                C0478L.beginSection("Layer#saveLayer");
                Utils.saveLayerCompat(canvas, this.rect, this.contentPaint);
                C0478L.endSection("Layer#saveLayer");
                clearCanvas(canvas);
                C0478L.beginSection("Layer#drawLayer");
                drawLayer(canvas, this.matrix, intValue);
                C0478L.endSection("Layer#drawLayer");
                if (hasMasksOnThisLayer()) {
                    applyMasks(canvas, this.matrix);
                }
                if (hasMatteOnThisLayer()) {
                    C0478L.beginSection("Layer#drawMatte");
                    C0478L.beginSection("Layer#saveLayer");
                    Utils.saveLayerCompat(canvas, this.rect, this.mattePaint, 19);
                    C0478L.endSection("Layer#saveLayer");
                    clearCanvas(canvas);
                    this.matteLayer.draw(canvas, matrix2, intValue);
                    C0478L.beginSection("Layer#restoreLayer");
                    canvas.restore();
                    C0478L.endSection("Layer#restoreLayer");
                    C0478L.endSection("Layer#drawMatte");
                }
                C0478L.beginSection("Layer#restoreLayer");
                canvas.restore();
                C0478L.endSection("Layer#restoreLayer");
            }
            recordRenderTime(C0478L.endSection(this.drawTraceName));
            return;
        }
        this.matrix.preConcat(this.transform.getMatrix());
        C0478L.beginSection("Layer#drawLayer");
    ...
    private void clearCanvas(Canvas canvas) {
        C0478L.beginSection("Layer#clearLayer");
        RectF rectF = this.rect;
        canvas.drawRect(rectF.left - 1.0f, rectF.top - 1.0f, rectF.right + 1.0f, rectF.bottom + 1.0f, this.clearPaint);
        C0478L.endSection("Layer#clearLayer");
    }
    ...
    private void intersectBoundsWithMask(RectF rectF, Matrix matrix2) {
        ...
        if (hasMasksOnThisLayer()) {
            ...
            while (i < size) {
                ...
                int i2 = C05132.$SwitchMap$com$airbnb$lottie$model$content$Mask$MaskMode[mask2.getMaskMode().ordinal()];
                if (i2 != 1 && i2 != 2) {
    ...
    static /* synthetic */ class C05132 {
        ...
        static {
            ...
            L_0x0083:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: com.airbnb.lottie.model.layer.BaseLayer.C05132.<clinit>():void");
        }
    }
