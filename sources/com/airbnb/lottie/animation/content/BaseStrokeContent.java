...
public abstract class BaseStrokeContent implements BaseKeyframeAnimation.AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    private final PathMeasure f48pm = new PathMeasure();
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        C0478L.beginSection("StrokeContent#draw");
        if (Utils.hasZeroScaleAxis(matrix)) {
            C0478L.endSection("StrokeContent#draw");
            return;
        }
        ...
        for (int i2 = 0; i2 < this.pathGroups.size(); i2++) {
            ...
            if (pathGroup.trimPath != null) {
                ...
                C0478L.beginSection("StrokeContent#buildPath");
                this.path.reset();
                for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
                    this.path.addPath(((PathContent) pathGroup.paths.get(size)).getPath(), matrix);
                }
                C0478L.endSection("StrokeContent#buildPath");
                C0478L.beginSection("StrokeContent#drawPath");
                canvas.drawPath(this.path, this.paint);
                C0478L.endSection("StrokeContent#drawPath");
            }
        }
        C0478L.endSection("StrokeContent#draw");
    }
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        C0478L.beginSection("StrokeContent#applyTrimPath");
        if (pathGroup.trimPath == null) {
            C0478L.endSection("StrokeContent#applyTrimPath");
            return;
        }
        this.path.reset();
        for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
        ...
        this.f48pm.setPath(this.path, false);
        float length = this.f48pm.getLength();
        while (this.f48pm.nextContour()) {
            length += this.f48pm.getLength();
        }
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            this.f48pm.setPath(this.trimPathPath, false);
            float length2 = this.f48pm.getLength();
            float f2 = 1.0f;
    ...
    public void getBounds(RectF rectF, Matrix matrix, boolean z) {
        C0478L.beginSection("StrokeContent#getBounds");
        this.path.reset();
        ...
        C0478L.endSection("StrokeContent#getBounds");
    }

    private void applyDashPatternIfNeeded(Matrix matrix) {
        C0478L.beginSection("StrokeContent#applyDashPattern");
        if (this.dashPatternAnimations.isEmpty()) {
            C0478L.endSection("StrokeContent#applyDashPattern");
            return;
        }
