...
public class PowerUsageFeatureProviderGoogleImpl extends PowerUsageFeatureProviderImpl {
    ...
    public boolean isChartGraphEnabled(Context context) {
        return isSettingsIntelligenceExist(context) && DatabaseUtils.isContentProviderEnabled(context);
    }

    public boolean isChartGraphSlotsEnabled(Context context) {
        if (!isSettingsIntelligenceExist(context) || !PhenotypeProxy.getFlagByPackageAndKey(context, "com.google.android.settings.intelligence", "BatteryUsage__is_time_slot_supported", false)) {
            return false;
        }
        return true;
    }
